...
export function DynamicStepForm(props: {
    ...
    if (existing?.length) {
      return existing.map((r) => {
        ...
        for (const k of Object.keys(r)) {
          row[k] = r[k] === undefined || r[k] === null ? "" : String(r[k]);
        }
        return row;
      });
    }
    return [emptyRow(schema.columns)];
  });
  ...
  function emptyRow(cols: FieldDef[]) {
    ...
    return (
      <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {schema.columns.map((c) => (
                    <td key={c.key} className="p-2">
                      <Input
                        ...
                        type={c.field_type === "number" || c.field_type === "integer" ? "number" : "text"}
                      />
                    </td>
                  ))}
                </tr>
        ...
        <Button
          type="button"
          variant="outline"
          onClick={() => setRows([...rows, emptyRow(schema.columns)])}
        >
          {t("process.addRow")}
        </Button>
